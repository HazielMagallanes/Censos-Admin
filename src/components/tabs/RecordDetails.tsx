    ...
    <div className="flex flex-col w-full h-full bg-white p-6 overflow-auto">
      ...
      <div className="flex flex-col md:flex-row border border-slate-200 rounded-sm min-h-[700px]">
        ...
        <div className="flex-1 flex flex-col">
          <div className="p-4 border-b border-slate-200 text-lg md:text-xl text-slate-800 font-medium">Visualización del registro censal</div>

          <div className="p-4 border-b border-slate-200 text-slate-700">Nombre del formulario</div>

          <div className="p-4 border-b border-slate-200 text-slate-700">ID: RC25</div>
          ...
          <div className="px-4 pb-4 flex flex-col gap-3">
            {formSections.map((section, sectionIdx) => (
              <div key={sectionIdx} className="flex flex-col gap-2">
                {/* Category Box */}
                <div className="border border-slate-200 rounded-md p-2.5 text-sky-600/80 bg-white w-full text-sm">{section.categoryName}</div>
                  ...
                  <div className="flex flex-col gap-2 pl-2">
                    {section.attributes.map((attr, attrIdx) => (
                      <div key={attrIdx} className="flex items-center gap-3">
                        ...
                        <div className="border border-slate-200 rounded-md p-2.5 text-sky-600/80 flex-1 bg-white text-sm">{attr}</div>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="w-full md:w-[350px] border-l border-slate-200 flex flex-col">
          <div className="p-4 border-b border-slate-200 text-lg text-slate-800 font-medium">Detalle del formulario</div>

          <div className="p-4 border-b border-slate-200 text-sm text-slate-700">Creada el: dd / mm / aaaa</div>
          ...
          <div className="p-4 border-b border-slate-200 text-sm text-slate-700 flex flex-col gap-4">
            ...
            <div className="flex gap-2 items-center">
              ...
              <span className="text-sky-800 font-medium cursor-pointer hover:underline">Visualizar formulario utilizado</span>
            </div>
          </div>

          {/* Warning Message Zone */}
          <div className="p-4 text-sm text-[#b20000] flex-1">Zona de alertas e información circunstancial.</div>

          {/* Bottom Action Area */}
          <div className="p-4 border-t border-slate-200 mt-auto">
            <Button className="w-full bg-[#b20000] hover:bg-[#8f0000] text-white rounded-md h-12 text-base font-normal shadow-none">Volver</Button>
          </div>
        </div>
      </div>
    </div>
